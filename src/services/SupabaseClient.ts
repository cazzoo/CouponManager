...
// Using an IIFE instead of top-level await
(function initSupabaseConfig() {
  if (isNode) {
    // Node.js environment (scripts)
    // Use synchronous require instead of dynamic import with await
    try {
      // In Node environment, require dotenv directly instead of using await import
